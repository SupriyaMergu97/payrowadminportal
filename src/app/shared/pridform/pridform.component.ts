...
@Component({
  selector: 'app-pridform',
  templateUrl: './pridform.component.html',
  styleUrls: ['./pridform.component.scss'],
  providers: [
    {
...
export class PridformComponent implements ControlValueAccessor, OnDestroy, OnInit, Validator {
  pridForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.defaultDate = this.datePipe.transform(currentDate, 'dd-MM-yyyy', 'en-US')!;
    // this.pridForm.reset();
    this.getSalesList()
    ...
    if (this.idProof !== '') {
      ...
      else { this.pridForm.patchValue({ emiratesId: this.idProof }) }
    }
    else {
  ...
  constructor(private fb: FormBuilder, private createAcnt: CreateAcntService, private noteService: NotificationService,
    private sales_serv: SalesPersonService, private datePipe: DatePipe, private encryption: SignatureEncryptionService) {
    this.pridForm = this.fb.group({
      ...
      emiratesDocument: [''],
      passportDocument: [''],
      emiratesId: new FormControl("", [Validators.minLength(15), Validators.maxLength(15)]),
  ...
  submit() {
    ...
    let reqhead = this.encryption.createHeader();
    const key = this.encryption.generateKey(reqhead.key)
    this.createAcnt.createPayrowId(fdata, reqhead.headers).subscribe(async (data) => {
  ...
  writeValue(value: any): void {
    ...
    if (value === null) { this.pridForm.reset(); }
  }

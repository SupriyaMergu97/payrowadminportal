...
export class EmployeeRelationsComponent implements OnInit {
  ...
  private loadScripts(): void {
    (function ($) {
      ...
      $("#side_menu_bar > ul > li.nav-item > a#li_employee_relations").addClass(
        "active",
      );
    })(jQuery);
  }
    ...
    this.emp.getAllDept(reqhead.headers).subscribe(async (res) => {
      if (res.success === true) {
        ...
        decryptedData.map((d: any) => {
          ...
          this.emp
            .getEmpbydeptId(d.deptId, reqheademp.headers)
            .subscribe(async (res) => {
              const encryptedDataemp = res.data; // Assuming encrypted data comes under 'data'
              const decryptedDataemp = JSON.parse(
                this.encryption.decodeData(encryptedDataemp, await keyemp),
              );
  ...
  getEmpbydeptId(deptId: any) {
    ...
    this.emp.getEmpbydeptId(deptId, reqhead.headers).subscribe(async (res) => {
      if (res.success === true) {
        const encryptedDataemp = res.data; // Assuming encrypted data comes under 'data'
        const decryptedDataemp = JSON.parse(
          this.encryption.decodeData(encryptedDataemp, await key),
        );
  ...
  submit() {
    ...
    formData.append(
      "nationalIdFile",
      this.empForm.get("nationalIdFile")?.value,
    );
    ...
    this.employeeDetails =
      event === "A to Z"
        ...
        : event === "Z to A"
          ...
          : event === "salary_low"
            ? _.orderBy(this.employeeDetails, ["salary"], ["asc"])
  ...
  updateOne(emp: any) {
    ...
    if (this.empForm.get("empImage")?.value === null) {
      ...
      let req = {
        data: this.encryption.encodeJsonObjectToHex(this.empForm.value),
      };
      let reqhead = this.encryption.createHeader();
      const key = this.encryption.generateKey(reqhead.key);
      this.emp.updateDept(req, reqhead.headers).subscribe(async (res) => {
  ...
  wps(emp: any) {
    ...
    const key = this.encryption.generateKey(reqhead.key);
    this.emp
      .wpsDetails(req, emp.empId, reqhead.headers)
      .subscribe(async (res) => {
        if (res.success === true) {
          const encryptedData = res.data; // Assuming encrypted data comes under 'data'
          const decryptedData = JSON.parse(

...
export class PridFormComponent implements ControlValueAccessor, OnDestroy, OnInit, Validator {
  ...
  onFileSelected(event: any) {
    ...
    // this.selectedFiles.push({"emiratesDocument":file});
  ...
  submit() {
    ...
    let reqhead = this.encryption.createHeader();
    const key = this.encryption.generateKey(reqhead.key)
    this.createAcnt.createPayrowId(fdata, reqhead.headers).subscribe(async (res) => {
      if (res.success == true) {
        const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
        const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
        ...
        this.noteService.showError(`${res.message}`, 'Error');
      }
    })
  }
  ngOnDestroy(): void {

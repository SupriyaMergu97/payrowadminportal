...
export class AcUserComponent implements OnInit {
	...
	constructor(private fb: FormBuilder, private http: HttpClient, private router: Router,
		private createAcnt: CreateAcntService, private smbservc: BarServiceService, private note_Servce: NotificationService,
	...
	merMastersList(mid: any) {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.gatewayServ.getposUserbyID(this.mid, reqhead.headers).subscribe(async res => {
			if (res.success === true) {
				const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
				const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
	...
	getposUserByID(id: any) {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.gatewayServ.getposUserbyID(id, reqhead.headers).subscribe(async res => {
			if (res.success === true) {
				const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
				const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
	...
	createPosUser() {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.gatewayServ.createposUser(req, reqhead.headers).subscribe(async res => {
			if (res.success === true) {
				const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
				const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
	...
	updateUser() {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.gatewayServ.updateposUserByID(req, this.tempData[0]._id, reqhead.headers).subscribe(async res => {
			if (res.success === true) {
				const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
				const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
				this.merMastersList(this.mid);
	...
	getTidbyMidList(mid: any) {
		...
		this.createAcnt.getTidList(mid, reqhead.headers).subscribe(async res => {
			if (res.success) {
				...
				this.createAcnt.getTransValuebyTid(reqheadtrans.headers).subscribe(async tData => {
					if (tData.success === true) {
						const encryptedDatatrans = tData.data;  // Assuming encrypted data comes under 'data'
						const decryptedDatatrans = JSON.parse(this.encryption.decodeData(encryptedDatatrans, await keytrans));
						decryptedData.map((d: any) => {
	...
	createTid() {
		if (this.isDisableInput == "text") {
			...
			let reqhead = this.encryption.createHeader();
			const key = this.encryption.generateKey(reqhead.key)
			this.createAcnt.createTerminalId(req, reqhead.headers).subscribe(async res => {
				if (res.success === true) {
					const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
					const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
			...
			let reqhead = this.encryption.createHeader();
			const key = this.encryption.generateKey(reqhead.key)
			this.createAcnt.createTidbyFile(this.tidFormData, reqhead.headers).subscribe(async res => {
				if (res.success === true) {
					const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
					const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
					this.getTidbyMidList(this.mid);
	...
	addServtoMerchant() {
		this.onCheckdData.map((serv: any) => {
			...
			let reqhead = this.encryption.createHeader();
			const key = this.encryption.generateKey(reqhead.key)
			this.createAcnt.addServtoMerchant(req, reqhead.headers).subscribe(async res => {
				if (res.success == true) {
					const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
					const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
					console.log('res');
	...
	getServDetails(id: any) {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.gatewayServ.getServofPosMerbyId(id, reqhead.headers).subscribe(async res => {
			if (res.success === true) {
				const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
				const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
				this.tempData = decryptedData;
	...
	delUser() {
		if (this.userType === "PosUser") {
			let reqhead = this.encryption.createHeader();
			const key = this.encryption.generateKey(reqhead.key)
			this.gatewayServ.delposUserbyID(this.tempData[0]._id, reqhead.headers).subscribe(async res => {
				if (res.success === true) {
					this.merMastersList(this.mid);
					const encryptedData = res.data;  // Assuming encrypted data comes under 'data'
					const decryptedData = JSON.parse(this.encryption.decodeData(encryptedData, await key));
	...
	onSelectCat(item: any) {
		...
		let reqhead = this.encryption.createHeader();
		const key = this.encryption.generateKey(reqhead.key)
		this.srvc_Cat.getServByCat(this.selectedCat, reqhead.headers).subscribe(async res => {
	...
	pridListDownload() {
		...
		const options = {
			...
			headers: ['PayRow ID', 'Name', 'Transaction Value', 'Activation Date', 'Status']
		};
	...
	MidListDownload() {
		...
		const options = {
			title: '', fieldSeparator: ',', quoteStrings: '"', decimalseparator: '.', showLabels: true, showTitle: true,
			headers: ['Merchant ID', 'Transaction Value', 'License Number', 'Company Name', , 'Activation Date', 'Status']
		};
		this.csvOptions = options;
	...
	tidListDownload() {
		...
		const options = {
			title: '', fieldSeparator: ',', quoteStrings: '"', decimalseparator: '.', showLabels: true, showTitle: true,
			headers: ['Terminal Id', 'Manager Id', 'Email ID', 'Mobile Number', 'Transaction Value', 'Activation Date', 'Status']
		};
		this.csvOptions = options;
		...
		this.merchantList.map((d: any) => {
			if (d._id == id) {
				...
				this.dialogData = {
					...
					content12: d.payRowId !== undefined && d.payRowId !== null ? d.payRowId : '',
					subHeader13: "",
					content13: "",
					header2: "Admin Details",
					subHeader21: "Admin ID",
					content21: d.adminDetails.length > 0 && d.adminDetails[0].adminId !== null ? d.adminDetails[0].adminId : '',
					subHeader22: "Admin Name",
					content22: d.adminDetails.length > 0 && d.adminDetails[0].adminName !== null ? d.adminDetails[0].adminName : '',
					subHeader23: "Admin Contact",
					content23: d.adminDetails.length > 0 && d.adminDetails[0].mobileNumber !== null ? d.adminDetails[0].mobileNumber : '',
					header3: "License Details",
					subHeader31: "License Number",
					content31: d.licenseDetails.licenseNumber !== undefined && d.licenseDetails.licenseNumber !== null ? d.licenseDetails.licenseNumber : '',
					subHeader32: "Company Name",
					content32: d.licenseDetails.companyName !== undefined && d.licenseDetails.companyName !== null ? d.licenseDetails.companyName : '',
				}
				// this.empForm.patchValue(d);
				// this.profileImg = `https://payrowdev.uaenorth.cloudapp.azure.com/adminbackend/distributor/download/${d.empImageId}`;
				// console.log(d.path, this.profileImg, 'image');
	...
	getTidList(id: any) {
		...
		this.createAcnt.getMerchantById(id).subscribe(data => {
					...
					"documentsData": [
						{
							...
							"documentPath": "payrowusers\\631f213d2cf7a9c7c70831b2\\bank1-1662984817056.jpg",
						...
						},
						{
							...
							"documentPath": "payrowusers\\631f213d2cf7a9c7c70831b2\\auth_img2-1662984675422.png",
							"createdDate": "2022-09-12T12:05:10.601Z",
						...
						},
						{
							...
							"documentPath": "payrowusers\\631f213d2cf7a9c7c70831b2\\aim-1662984675434.png",
							"createdDate": "2022-09-12T12:05:10.601Z",
					...
					"terminalsInfo": [
						{
							...
							"pin": 2211,
							"status": "Active",
							"reqEncrypt": "AtLRmCcqo2jwANlcl3TgFrp/VPDRaI1G+xgrKmSvjxbr5C/VrTHBpZsdJaPpxViAzf4rpAzHKV5kP02hPJ6dJQ==",
			...
			result.forEach((mData: any) => {
				mData.terminalsInfo.map((tidData: any) => {
					let tids = {
						tidInfo: tidData, mid: mData.bankMID, activationDate: "pending", address: mData.addressDetails,
						emailId: mData.emailId, firstName: mData.firstName, tid: "pending"
					}
					this.staffListA.push(tids)
				})
			});

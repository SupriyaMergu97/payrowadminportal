...
export class MidReqFormComponent implements OnInit {
	...
	img2: any
	ejsContent:  SafeHtml = '';
	constructor(private createAcnt: CreateAcntService, private sanitizer: DomSanitizer) { }
	ejsUrl: string;
	ngOnInit(): void {
		this.createAcnt.getForm().subscribe(content => {
			this.ejsContent = this.sanitizer.bypassSecurityTrustHtml(content);
		  }, error => {
			console.error('Error fetching minimal content:', error);
		  });
		
		// .subscribe((content) => {
		// 	this.ejsUrl = 'https://payrowdev.uaenorth.cloudapp.azure.com/merchant/merchantAgreement';
		// 	this.ejsContent = content;
		// 	// console.log('ejscontent',this.ejsContent)
		// })
